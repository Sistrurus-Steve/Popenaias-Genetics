...
Samp_log <- read.csv('pp_no_sd_enviro.csv')
Coord <- Samp_log[,3:4]
Coord <- as.matrix(Coord)
SNPs <- read.delim('pp_no_sd_SNPs.csv', header = FALSE)
Env <- Samp_log[,5:28]
Loci <- read.lfmm(input.file = 'pp_no_sdc.lfmm')
...
write.csv(MSR, 'pp_no_sd_MSPcandidates.csv', row.names = FALSE)
