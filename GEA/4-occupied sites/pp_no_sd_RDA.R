...
Gtypes <- 'pp_no_sdc.lfmm' # The 012 file with no missing data (it's been imputed)
Env <- 'pp_no_sd_enviro.csv' # The popmap and environmental data 
Snps <- 'pp_no_sd_SNPs.csv' # A file with your SNPs
Outputprefix <- 'pp_no_sd_RDA' # Output prefix to add to file names 
###############################################
...
Cor <- findCorrelation(cor(EnvVars[,5:28]), cutoff = 0.75)
SelectedVars <- EnvVars[,5:28]
...
colnames(SelectedVars) <- c("PWQ","OM","Flow","recharge")
...
colnames(foo) <- c("PWQ","OM","Flow","recharge")
...
write.csv(cand, "pp_no_sd_CanidateSNPs_Full.csv")
##############################
##### Compare to SNP Map #####
##############################

SNPsss <- read.csv('pp_no_sd_SNPssss.csv', header = TRUE, sep = ",")
...
Mapped <- inner_join(candSNP, SNPsss, by = "Position")
...
legend("topleft", legend= c("PWQ","OM","Flow","recharge"), bty="n", col="gray32", pch=21, cex=1, pt.bg=bg) #adds legend
# adds legend
